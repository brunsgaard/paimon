...
The committer sets `write.data.path` on the Iceberg table to the Paimon table location, so a
catalog that vends credentials scopes them to the data files. Data files under Paimon external
paths are outside that scope.
